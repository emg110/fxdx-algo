...
from src.smart_contracts import ExchangePlatformASC1, exchange_escrow


class ExchangePlatform:
    def __init__(
            self, admin_pk, admin_address, exchange_id, client
    ):
        ...
        self.teal_version = 4
        self.exchange_platform_asc1 = ExchangePlatformASC1()
    ...
    def escrow_bytes(self):
        ...
        escrow_fund_program_compiled = compileTeal(
            exchange_escrow(app_id=self.app_id, asa_id=self.exchange_id),
            mode=Mode.Signature,
            version=4,
        )
    ...
    def app_initialization(self, exchange_owner_address):
        approval_program_compiled = compileTeal(
            self.exchange_platform_asc1.approval_program(),
            mode=Mode.Application,
            version=4,
        )

        clear_program_compiled = compileTeal(
            self.exchange_platform_asc1.clear_program(),
            mode=Mode.Application,
            version=4
        )
        ...
        app_args = [
            decode_address(exchange_owner_address),
        ...
        app_transaction = ApplicationTransactionRepository.create_application(
            ...
            global_schema=self.exchange_platform_asc1.global_schema,
            local_schema=self.exchange_platform_asc1.local_schema,
            app_args=app_args,
            foreign_assets=[self.exchange_id],
        )
    ...
    def initialize_escrow(self):
        app_args = [
            self.exchange_platform_asc1.AppMethods.initialize_escrow,
    ...
    def make_sell_offer(self, sell_price: int, exchange_owner_pk):
        app_args = [self.exchange_platform_asc1.AppMethods.make_sell_offer, sell_price]

        app_call_txn = ApplicationTransactionRepository.call_application(
            client=self.client,
            caller_private_key=exchange_owner_pk,
            app_id=self.app_id,
            on_complete=algo_txn.OnComplete.NoOpOC,
            app_args=app_args,
            sign_transaction=True,
    ...
    def buy_nft(self,
                exchange_owner_address, buyer_address, buyer_pk, buy_price):
        # 1. Application call txn
        app_args = [
            self.exchange_platform_asc1.AppMethods.buy
        ]
        ...
        asa_buy_payment_txn = PaymentTransactionRepository.payment(client=self.client,
                                                                   ...
                                                                   receiver_address=exchange_owner_address,
                                                                   amount=buy_price,
        ...
        asa_transfer_txn = ASATransactionRepository.asa_transfer(client=self.client,
                                                                 ...
                                                                 revocation_target=exchange_owner_address,
                                                                 sender_private_key=None,
                                                                 sign_transaction=False)
