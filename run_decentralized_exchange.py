...
decentralized_marketplace_contract = ExchangePlatformASC1()
...
# Create ASA.
exchange_1_service = ExchangeService(exchange_creator_pk=acc_pk,
                           exchange_creator_address=acc_address,
                           unit_name="TOK",
                           asset_name="Tokility")

nft1_id = exchange_1_service.create_nft(client)
...
print(f'Deployed ExchangePlatform app with app_id: {app_id}')

# Initialize escrow.

escrow_fund_program_compiled = compileTeal(exchange_escrow(app_id=app_id,
                                                      asa_id=nft1_id),
                                           mode=Mode.Signature,
                                           version=4)
...
exchange_1_service.change_exchange_credentials_txn(client, escrow_fund_address)
...
print(f'Escrow initialized in the ExchangePlatform ASC1.')
...
opt_in_txn = ASATransactionRepository.asa_opt_in(client=client,
                                                 sender_private_key=exchange_buyer_pk,
                                                 asa_id=exchange_1_service.asa_id)
...
app_call_txn = ApplicationTransactionRepository.call_application(client=client,
                                                                 caller_private_key=exchange_buyer_pk,
                                                                 app_id=app_id,
                                                                 on_complete=algo_txn.OnComplete.NoOpOC,
                                                                 app_args=app_args,
...
asa_buy_txn = PaymentTransactionRepository.payment(client=client,
                                                   sender_address=exchange_buyer_address,
...
asa_transfer_txn = algo_txn.AssetTransferTxn(sender=escrow_fund_address,
                                             sp=sp,
                                             receiver=exchange_buyer_address,
                                             amt=1,
                                             index=nft1_id,
...
app_call_txn_signed = app_call_txn.sign(exchange_buyer_pk)

asa_buy_txn_signed = asa_buy_txn.sign(exchange_buyer_pk)
