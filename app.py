...
from src.repository.exchange_platform_repository import ExchangePlatformRepository
import time
import algosdk
...
if "algobot" not in st.session_state:
    st.session_state.algobot = ExchangeService(exchange_creator_pk=admin_pk,
                                          exchange_creator_address=admin_address,
                                          client=client,
                                          unit_name="bot",
                                          asset_name="Algobot 76",
                                          exchange_url="https://gateway.pinata.cloud/ipfs/QmZBiFNgb6hWj4JYwXc3PL5QqsoHYaGd4dNv2RWZZzSbjY")

if "algoanna" not in st.session_state:
    st.session_state.algoanna = ExchangeService(exchange_creator_pk=admin_pk,
                                           exchange_creator_address=admin_address,
                                           client=client,
                                           unit_name="goan",
                                           asset_name="Al Goanna 25",
                                           exchange_url="https://ipfs.io/ipfs/QmWxUWbMRfG1fvdopnk1erw6EnY9D8ANrhWEkMfApgXMr5")


# 1. Mint NFTS

def mint_algobot_nft():
    if st.session_state.algobot.exchange_id is None:
...
def mint_algoanna_nft():
    if st.session_state.algoanna.exchange_id is None:
...
if st.session_state.algobot.exchange_id is None:
    ...
    exchange_id = st.session_state.algobot.exchange_id
    if st.session_state.algobot_image is None:
        st.session_state.algobot_image = exchange_repository.exchange_image(exchange_id)

    buttons[0].image(st.session_state.algobot_image,
                     caption=f"Algobot 76 with exchange_id: {exchange_id}",
                     use_column_width=True)

    if "algobot_market" not in st.session_state:
        st.session_state.algobot_market = ExchangePlatform(admin_pk=admin_pk,
...
if st.session_state.algoanna.exchange_id is None:
    ...
    exchange_id = st.session_state.algoanna.exchange_id
    if st.session_state.algoanna_image is None:
        st.session_state.algoanna_image = exchange_repository.exchange_image(st.session_state.algoanna.exchange_id)
    buttons[1].image(st.session_state.algoanna_image,
                     caption=f"Mint Al Goanna 025 with exchange_id: {exchange_id}",
                     use_column_width=True)

    if "algoanna_market" not in st.session_state:
        st.session_state.algoanna_market = ExchangePlatform(admin_pk=admin_pk,
                                                          admin_address=admin_address,
                                                          exchange_id=exchange_id,
                                                          client=client)
...
if can_deploy and st.session_state.should_deploy_apps:
    ...
    tx_id = st.session_state.algobot_market.app_initialization(exchange_owner_address=admin_address)
    st.session_state.transactions.append(f"ASC1 for Algobot 76 deployed in {tx_id}")

    tx_id = st.session_state.algoanna_market.app_initialization(exchange_owner_address=admin_address)
    st.session_state.transactions.append(f"ASC1 for Al Goanna 025 deployed in {tx_id}")

    tx_id = st.session_state.algobot.change_exchange_credentials_txn(
    ...
    tx_id = st.session_state.algoanna.change_exchange_credentials_txn(
...
def sell_algobot(sell_price: int):
    exchange_owner_pk = st.session_state.algobot_owner[0]
    tx_id = st.session_state.algobot_market.make_sell_offer(sell_price=sell_price,
                                                            exchange_owner_pk=exchange_owner_pk)
...
def buy_algobot(buy_price: int, owner_address):
    ...
    tx_id = st.session_state.algobot_market.buy_nft(exchange_owner_address=owner_address,
...
def sell_algoanna(sell_price: int):
    exchange_owner_pk = st.session_state.algoanna_owner[0]
...
def buy_algoanna(buy_price: int, owner_address):
    ...
    tx_id = st.session_state.algoanna_market.buy_nft(exchange_owner_address=owner_address,
                                                     buyer_address=buyer_1_address,
                                                     buyer_pk=buyer_1_pk,
                                                     buy_price=buy_price)
...
if st.session_state.app_is_deployed:
    ...
    algobot_app_state = ExchangePlatformRepository.load_app_state(st.session_state.algobot_market.app_id)
    ...
    if algobot_app_state["APP_STATE"] == 1:
        ...
        exchange_price = algobot_app_state["ASA_PRICE"]
        exchange_seller = algobot_app_state["ASA_OWNER"]
        st.success(f"The Algobot 76 is on sale for: {exchange_price} micro algos by: {exchange_seller}")
        _ = st.button(f'Buy Algobot 76', on_click=buy_algobot,
                      args=(exchange_price, exchange_seller))

    algoanna_app_state = ExchangePlatformRepository.load_app_state(st.session_state.algoanna_market.app_id)
    ...
    if algoanna_app_state["APP_STATE"] == 1:
        ...
        exchange_price = algoanna_app_state["ASA_PRICE"]
        exchange_seller = algoanna_app_state["ASA_OWNER"]
        st.success(f"The Al Goanna 025 is on sale for: {exchange_price} micro algos by: {exchange_seller}")
